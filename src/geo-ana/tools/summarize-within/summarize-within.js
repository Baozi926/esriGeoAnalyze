...
define([
    ...
    '../ToolBase',
    '../../config'
], function (FeatureLayer, ArrayUtil, on, all, domConstruct, lang, domClass, domStyle, esriConfig, esriRequest, _WidgetBase, _TemplatedMixin, declare, template, arrayUtil, summarizeWithinTask, arcgisUtil, ToolBase, geoAnaConfig) {
    var widget = declare('caihm.widgets.summarize-within', [
            ...
            for (var j = 0; j < this.data.currentServicesInfo.length; j++) {
                ...
                for (var i = 0; i < v.layers.length; i++) {
                    if (evt.target.value != (services.id + "?" + v.layers[i].id)) {
                        domConstruct.create('option', {
                            value: services.id + "?" + v.layers[i].id,
                            innerHTML: services.title + "-" + v.layers[i].name
                        }, this.huizongNode)
                    }
        ...
        huizongLayerChange(evt) {
            ...
            if (evt.target.value != "0") { //如果不是“请选择”
                var layerId = evt
                    .target
                    .value
                    .split("?")[0];
                ...
                if (Xurl.geometryType == "point") {
                    ...
                    domConstruct.empty(this.DanWei);
                    ArrayUtil.forEach(arcgisUtil.polygonUnits, function (v) {
                        domConstruct.create('option', {
                            innerHTML: v.name,
                            value: v.value
                        }, this.DanWei);
                ...
                for (var j = 0; j < this.data.currentServicesInfo.length; j++) {
                    ...
                    for (var i = 0; i < v.layers.length; i++) {
                        if (evt.target.value != (services.id + "?" + v.layers[i].id)) {} else {
                ...
                var fields = temServices.fields;

                ArrayUtil.forEach(fields, function (v) { //只加载数字类型
                ...
                for (var i = 0; i < selects.length; i++) {
                    if (selects[i].id.substr(0, 6) == "ZiDuan" && parseInt(selects[i].id.substr(6)) > evt.target.id.substr(10)) {
                        //所有字段
                        ...
                        ArrayUtil.forEach(this.data.avaliableFields, function (v) {
                            domConstruct.create('option', {
                                innerHTML: v.alias,
                                value: v.name
                            }, selects[i]);
                        }, this)
                    } else if (selects[i].id.substr(0, 11) == "TongJiXinXi" && parseInt(selects[i].id.substr(11)) > evt.target.id.substr(10)) {
                        //加载统计类型
                        domConstruct.empty(selects[i]);
                        ...
                        ArrayUtil.forEach(arcgisUtil.TongJiFangShiUnits, function (v) {
                            domConstruct.create('option', {
                                innerHTML: v.name,
                                value: v.value
                            }, selects[i]);
                ...
                ArrayUtil.forEach(avaliableFieldsAll, function (v) {
                    if (v.alias != "OBJECTID") {
                        domConstruct.create('option', {
                            innerHTML: v.alias,
                            value: v.name
        ...
        ZiDuanChange(evt) {
            ...
            if (value != "0") { //两者皆被选
                ...
                if (nextNum > nowNum) {
                    ...
                    //加一个关闭按钮
                    var liClose = domConstruct.create('p', {
                        class: "closestyle",
                        id: "closestyle" + (nextNum - 1),
                        innerHTML: "删除"
                    }, this.summaryFields_Node)
                    //加一个字段框
                    var liZiDuan = domConstruct.create('select', {
                        class: "aliSelect",
                        id: "ZiDuan" + nextNum
                    }, this.summaryFields_Node)
                    ...
                    ArrayUtil.forEach(this.data.avaliableFields, function (v, k) {
                        domConstruct.create('option', {
                            value: v.name,
                            innerHTML: v.alias
                        }, liZiDuan)
                    ...
                    //加一个统计信息框
                    var liTongJiXinXi = domConstruct.create('select', {
                        class: "aliSelect",
                    ...
                    ArrayUtil.forEach(arcgisUtil.TongJiFangShiUnits, function (v, k) {
                        domConstruct.create('option', {
                            value: v.value,
                            innerHTML: v.name
                        }, liTongJiXinXi)
                    ...
                    //添加点击事件
                    dojo.connect(liZiDuan, "onchange", lang.hitch(this, function (evt) {
                        this.ZiDuanChange(evt);
                    }));
                    //添加字段变化事件
                    dojo.connect(liTongJiXinXi, "onchange", lang.hitch(this, function (evt) {
                        this.TongJiXinXiChange(evt);
                    }));
                    //添加点击事件
                    dojo.connect(liClose, "onclick", lang.hitch(this, function (evt) {

                        var id1 = "ZiDuan" + evt
                            .target
                            .id
                            .substr(10);
                        var id2 = "TongJiXinXi" + evt
                            .target
                            .id
                            .substr(10);
                        ...
                        document
                            .getElementById(id1)
                            .parentNode
                            .removeChild(document.getElementById(id1));
                        document
                            .getElementById(id2)
                            .parentNode
                            .removeChild(document.getElementById(id2));
                        document
                            .getElementById(id3)
                            .parentNode
                            .removeChild(document.getElementById(id3));
                        ...
                        for (var i = 0; i < selects.length; i++) {
                            if (selects[i].id.substr(0, 6) == "ZiDuan" && parseInt(selects[i].id.substr(6)) > evt.target.id.substr(10)) {
                                ...
                                selects[i].id = "ZiDuan" + (parseInt(selects[i].id.substr(6)) - 1);
                            } else if (selects[i].id.substr(0, 11) == "TongJiXinXi" && parseInt(selects[i].id.substr(11)) > evt.target.id.substr(10)) {

                                selects[i].id = "TongJiXinXi" + (parseInt(selects[i].id.substr(11)) - 1);
                            }
                        }

                        for (var i = 0; i < ps.length; i++) {
                            if (ps[i].id.substr(0, 10) == "closestyle" && parseInt(ps[i].id.substr(10)) > evt.target.id.substr(10)) {

                                ps[i].id = "closestyle" + (parseInt(ps[i].id.substr(10)) - 1);
                            }
                        }
                    }));
                }
        ...
        runTask() {
            ...
            if (state.valid) { //如果填写正确
                ...
                var dfd = new summarizeWithinTask().run({ //调用task
