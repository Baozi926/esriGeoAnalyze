...
], function (esriConfig, dom, lang, domConstruct, Pane, css, arcgisUtil, TaskMonitor) {
  ...
  Widget.prototype = {
    ...
    init: function (param) {
      ...
      if (!this.user.username) {
        domConstruct.empty(param.srcNodeId);
        var loginUrl = window.location.origin + '/geoplat/signin.html?redirectUrl=' + window.location.href
        domConstruct.create('div', {
          innerHTML: '<div class="text" >只有登录，且level 2级别的用户才具有空间分析的权限</div><a class="btn spatial-login' +
              '" href="' + loginUrl + '"><i class="icon fa-user-circle"></i><span class="txt">登录</span></a>',
