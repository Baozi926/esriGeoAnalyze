define([
  ...
  var REQUIRE_URL_PERFIX = 'modules/geoAnalyze/'; //modules/geoAnalyze/
  REQUIRE_URL_PERFIX = 'src/geoAnalyze/'; //modules/geoAnalyze/
  var clazz = declare(
    'modules.geoAnalyze.Pane',
    [_WidgetBase, _TemplatedMixin],
    {
      ...
      startup: function() {
       ...
       if(trigger){
        ...
        this.switchPane('list');
        this.render();
        TaskMonitor.getInstance().setDetailNode(
          domConstruct.create(
            'div',
            {
      ...
      render: function() {
        if (!this.data.activeTool) {
          ...
          ArrayUtil.forEach(
            this.data.tooleList,
            function(v, k) {
              var rootContainer = domConstruct.create(
                'div',
                {
                  innerHTML: '',
                  ...
                  var dom = domConstruct.create(
                    'a',
                    {
                      innerHTML:
                        ...
                        '</span>',
                      className:
                        'tool-item cell service-item ' + (vv.icon || ''),
